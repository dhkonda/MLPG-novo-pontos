...
!esta versao do RPIM evita a inversao da matriz G e eh mais eficiente computacionalmente.    

SUBROUTINE RPIM(JJ,XB,YB,NPT,NPSUP,X,Y,FG,MON,TMON,Sa,Sb,Fi,dFiX,dFiY,ddFiX,ddFiY,ddFiXY)
    IMPLICIT NONE                           
    
INTEGER::i,J,K,JJ,NPT,NPSUP,MON,TMON
...
REAL*8,DIMENSION(NPSUP,NPSUP)::R,DIST,Ri,IDENT,Sa
REAL*8,DIMENSION(1,NPSUP)::dRX,dRY,ddRx,ddRy,ddRxy
REAL*8,DIMENSION(NPSUP,TMON)::P
REAL*8,DIMENSION(TMON,NPSUP)::PtRi,Sb
REAL*8,DIMENSION(TMON,TMON)::PtRiP,IDENT2,PtRiPi
...
REAL*8,DIMENSION(TMON+NPSUP,TMON+NPSUP)::G,Gi !,dGx,dGy,dGix,dGiy
...
!calculo de R inversa
DO i=1,NPSUP
    IDENT=0.d0
    IDENT(i,i)=1.d0
    CAll r8mat_fs(NPSUP,R,IDENT(:,i),Ri(:,i),JJ)
ENDDO
...
DO i=1,TMON
    DO J=1,NPSUP
        DO K=1,NPSUP
            PtRi(i,J)=P(K,i)*Ri(K,J)+PtRi(i,J)
        ENDDO
    ENDDO
ENDDO
...
!calculo da inversa de PtRiP
DO i=1,TMON
    IDENT2=0.d0
    IDENT2(i,i)=1.d0
    CAll r8mat_fs(TMON,PtRiP,IDENT2(:,i),PtRiPi(:,i),JJ)
ENDDO

!calculo de Sb
Sb=MATMUL(PtRiPi,PtRi)

!calculo de Sa
Sa=-MATMUL(TRANSPOSE(PtRi),Sb) 
Sa=Ri+Sa
...
DO i=1,NPSUP
    DO J=1,NPSUP
        FiRPIM(i)=R(1,J)*Sa(J,i)+FiRPIM(i)
    ENDDO
ENDDO
...
DO i=1,NPSUP
    DO J=1,TMON
        FiRPIM(i)=M(1,J)*Sb(J,i)+FiRPIM(i)
    ENDDO
ENDDO
...
DO i=1,NPSUP
    DO J=1,NPSUP
        dFiRPIMx(i)=dRx(1,J)*Sa(J,i)+dFiRPIMx(i)
        dFiRPIMy(i)=dRy(1,J)*Sa(J,i)+dFiRPIMy(i)    

        ddFiRPIMx(i)=ddRx(1,J)*Sa(J,i)+ddFiRPIMx(i)
        ddFiRPIMy(i)=ddRY(1,J)*Sa(J,i)+ddFiRPIMy(i)    
        ddFiRPIMxy(i)=ddRXY(1,J)*Sa(J,i)+ddFiRPIMXY(i)    
    ENDDO
ENDDO
...
DO i=1,NPSUP
    DO J=1,TMON
        dFiRPIMx(i)=dMx(1,J)*Sb(J,i)+dFiRPIMx(i)
        dFiRPIMy(i)=dMy(1,J)*Sb(J,i)+dFiRPIMy(i)

        ddFiRPIMx(i)=ddMX(1,J)*Sb(J,i)+ddFiRPIMx(i)
        ddFiRPIMy(i)=ddMY(1,J)*Sb(J,i)+ddFiRPIMy(i)
        ddFiRPIMxy(i)=ddMXY(1,J)*Sb(J,i)+ddFiRPIMxy(i)
    ENDDO
ENDDO
